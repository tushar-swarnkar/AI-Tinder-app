...
const fetchRandomProfile = async () => {
  const response = await fetch('http://localhost:8082/profiles/random');
  if (!response.ok) {
...
const saveSwipe = async (profileId) => {
  const response = await fetch('http://localhost:8082/matches', {
    method: 'POST',
    headers: {
...
const fetchConversation = async (conversationId) => {
  ...
  const response = await fetch(`http://localhost:8082/conversations/${conversationId}`);
  if (!response.ok) {
